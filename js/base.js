...
function creerCylinder(nom, rayon, heigth, subdivisions){
	var geo  = new THREE.CylinderGeometry(rayon,rayon, heigth, subdivisions, subdivisions) ; 
	var material = new THREE.ShaderMaterial({wireframe: true, transparent: true});
	var mesh = new THREE.Mesh(geo, material) ; 
	mesh.name = nom ; 
	return mesh ;  
}

function creerGraphe(nom, cylrad, cylheight, sphrad, subdivisions, sphqnt){
	var geo  = new THREE.CylinderGeometry(cylrad, cylrad, cylheight, subdivisions, subdivisions); 
	var material = new THREE.ShaderMaterial({wireframe: true, transparent: true});
	var mesh = new THREE.Mesh(geo, material) ;
	var groupe = new THREE.Group() ; 
	groupe.name = nom ;
	...
	for (i = 0; i < sphqnt; i++){
		var sphgeo  = new THREE.SphereGeometry(sphrad, subdivisions, subdivisions) ; 
		var colors = new THREE.Color( getRandomInt(0,1),getRandomInt(0,1), getRandomInt(0,1) );	
		var sphMaterial = new THREE.MeshLambertMaterial({color: colors})
		var sphmesh = new THREE.Mesh(sphgeo, sphMaterial) ;
		sphmesh.position.set(getRandomInt(-cylrad/2.0,cylrad/2.0),getRandomInt(-cylheight/2.0,cylheight/2.0),getRandomInt(-cylrad/2.0,cylrad/2.0)) ; 
		groupe.add(sphmesh);
	}
	return groupe;
}
