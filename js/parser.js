...
function parser(){
	...
	for(var i=0; i<objets.length; i++){
		...
		if(_obj.type == "cylinder"){
			console.log("CYLINDER : ",nom) ;
			console.log(_obj) ;
			var rayon = params.rayon ; 
			var subdiv = params.subdivisions ;   
			var h = params.height;
			var cyl = creerCylinder(nom, rayon, h, subdiv) ; 
			enregistrerDansAnnuaire(nom,cyl) ; 
		} else 
		...
		if(_obj.type == "sculture"){
			var cylinderRadius = params.cylrad;
			var cylinderHeight = params.cylheight;
			var sphereRadius = params.sphrad;
			var subdiv = params.subdivisions;
			var sphereQuantity = params.sphqnt;
			var sculture = creerGraphe(nom, cylinderRadius, cylinderHeight, sphereRadius, subdiv, sphereQuantity);
			enregistrerDansAnnuaire(nom, sculture) ;
		} else
		if(_obj.type == "texte"){
			var largeur = params.largeur ;
			var hauteur = params.hauteur ; 
